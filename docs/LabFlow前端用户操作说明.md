...
## 补充（2026-02-11）：停止训练任务

当你在「6) 任务轮询状态」看到任务为 `queued` 或 `running` 时，页面会显示按钮 **停止训练任务**。
...
预期结果：
- 队列中的任务会立即取消。
- 正在运行的训练会发送停止信号，随后任务状态变为 `canceled`。

常见情况：
- 刚提交训练时日志可能暂时为空，这是正常现象；系统会持续轮询，后续有内容会自动显示。
- 已结束任务（`success/failed/canceled`）不能再次停止。
